...
def get_new_messages(service):
    send_message(service, "MAO Send New Messages", "Task", "")
    while True:
...
def main():
    ...
    if code == 1:
        username = sys.argv[2]
        password = sys.argv[3]
        send_message(serviceP, "Login MAO " + username + " " + password, "Task", "")
        while(True):
            time.sleep(3)
        ...
        filepath = sys.argv[4]
        note = str(sys.argv[5])
        send_message(serviceP, note, "MAO Submission " + username + " " + password, filepath)
        while(True):
            time.sleep(3)
        ...
        send_message(serviceP, "MAO Hours", "Task", "")
        while(True):
            time.sleep(3)
            msgs = search_messages(serviceP, "MAO Hours")
            if msgs != []:
        ...
        send_message(serviceP, "MAO Opps", "Task", "")
        while(True):
            time.sleep(3)
            msgs = search_messages(serviceP, "MAO Opps")
            if msgs != []:
                get_attachment(serviceP, msgs[0]['id'], "Temp")
                mark_as_read(serviceP, msgs[0])
                break
    elif code == 17:
        ai_voice("ALERT! You are currently on probation for missing or late hours! Please contact an MAO officer to submit late hours and remove the probation tag.")
